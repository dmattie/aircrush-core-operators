...
#    rm -f $TARGET/registration/*
    ...
    if [[ -d $TARGET/registration ]];then
     echo   rmdir $TARGET/registration
    fi
...
if [[ ! -f ${REFERENCE} && -f ${REFERENCEmgz} ]];then
  ...
  echo  mri_convert -rt nearest -nc -ns 1 $REFERENCEmgz $REFERENCE
fi
if [[ ! -f $REFERENCE ]];then
...
echo cp $SOURCE_dwi $TARGET
echo cd $TARGET
echo fslsplit $SOURCE_dwi 
echo fslsplit $SOURCE_dwi #sub-${SUBJECT}_ses-${SESSION}_dwi${TIMEPOINT}.nii.gz
res=$?
if [[ $res != 0 ]];then
...
function flirt_ref() {
  ...
  echo $1  
  if [[ $BET2 -eq 0 ]];then
echo     flirt -in $1 -ref $REFERENCE -omat $fbase.RegTransform4D -out reg2ref.$fbase.nii
  else
    echo bet2 $1 bet_$1 -f 0.2
    echo flirt -in bet_$1 -ref $REFERENCE -omat $fbase.RegTransform4D -out reg2ref.$fbase.nii
  fi
...
echo ls vol*.n* | xargs -n1 -I@ -P$PARALLELISM bash -c 'fbase=`echo @|cut -f 1 -d.`;flirt -in @ -ref '$REFERENCE' -omat $fbase.RegTransform4D -out reg2ref.$fbase.nii'

echo fslmerge -a reg2brain_unmasked.data.nii reg2ref.*
#Remove anything outside of the reference image
echo fslmaths $REFERENCE -bin binary_brainmask.nii
echo fslmaths reg2brain_unmasked.data.nii -mul binary_brainmask.nii.gz reg2brain.data.nii
...
echo fslmaths $TARGET_FILE -mul binary_brainmask.nii.gz reg2brain_hardi.data.nii
...
if [[ ! -f "reg2brain.data.nii.gz" ]];then
    ...
    echo rm --force vol*
    echo rm --force bet_vol*
    echo rm --force reg2ref*
    echo rm --force core.*    
